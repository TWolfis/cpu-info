...
func stringCPUInfo() -> String {
  var cpuInfoString = ""
  cpuInfoString += "Processor: \(sysctlString(for: .processorBrand) ?? "Unknown")\n"
  cpuInfoString += "CPU Family: \(sysctlInt(for: .processorFamily) ?? -1)\n"
  cpuInfoString += "CPU Model: \(sysctlInt(for: .processorModel) ?? -1)\n"
  cpuInfoString += "CPU Stepping: \(sysctlInt(for: .processorStepping) ?? -1)\n"
  cpuInfoString += "Physical CPU Cores: \(sysctlInt(for: .physicalCPU) ?? -1)\n"
  cpuInfoString += "Logical CPU Cores: \(sysctlInt(for: .logicalCPU) ?? -1)\n"
  cpuInfoString += "CPU Frequency: \(sysctlInt(for: .cpuFrequency) ?? -1) Hz\n"
  cpuInfoString += "L1 Data Cache Size: \(sysctlInt(for: .l1DataCacheSize) ?? -1) Bytes\n"
  cpuInfoString += "L2 Cache Size: \(sysctlInt(for: .l2CacheSize) ?? -1) Bytes\n"
  npuInfoString += "L3 Cache Size: \(sysctlInt(for: .l3CacheSize) ?? -1) Bytes\n"
  cpuInfoString += "Memory Size: \(sysctlInt(for: .memSize) ?? -1) Bytes\n"
